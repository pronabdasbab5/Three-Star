                  ...
                  <div class="x_content">
                    ...
                    <table id="datatable-responsive" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
                      ...
                      <?php
                        ...
                        $sql_orders = "SELECT `orders`.*, `user`.`name`, `user`.`contact_no` FROM `orders`, `user` WHERE `orders`.`user_id` = `user`.`id` AND `orders`.`status`=1 ORDER BY `orders`.`id` DESC";
                        ...
                        if ($result_orders = $connection->query($sql_orders)){
                            ...
                            while($row_orders = $result_orders->fetch_assoc()) {
                                ...
                                if($row_orders['status'] == 1)
                                    $status = "<a class=\"btn btn-success\">New Order</a>";
                                ...
                                print "<td>$status</td>";
                                print "<td><a href=\"status/accept_order.php?order_id=$row_orders[id]\" class=\"btn btn-primary\">Accept</a><a href=\"status/cancel_order.php?order_id=$row_orders[id]\" class=\"btn btn-warning\">Cancel</a><a href=\"invoice.php?order_id=$row_orders[id]\" class=\"btn btn-danger\" target=\"_blank\">Details</a></td>";
                                print "</tr>";
                            }
                        }
                        ?> 
